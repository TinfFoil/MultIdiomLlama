...
def select_demonstrations(instruction_lang, demonstrations):
    """
    Select demonstrations matching criteria:
    1. Input language is DIFFERENT from the instruction language
    2. Preferably contains language-specific idiom detection markers
    ...
    # Possible input languages for cross-lingual demonstrations
    all_languages = ['en', 'it', 'pt']
    
    # Remove the instruction language from possible input languages
    input_languages = [lang for lang in all_languages if lang != instruction_lang]
    ...
    # First pass: find demonstrations with the right input language and markers
    for demo in demonstrations:
        try:
            # Check input languages
            input_found = False
            for input_lang in input_languages:
                input_key = f"input_{input_lang}"
            ...
            # If there's a language marker match, add the demonstration
            if lang_match:
    ...
    # If not enough demos found with markers, fall back to any cross-lingual demos
    if not matching_demos:
        print(f"Warning: No marked demonstrations found. Using cross-lingual approach.")
        
        for demo in demonstrations:
            try:
                # Check for input in a different language
                input_found = False
                for input_lang in input_languages:
                    input_key = f"input_{input_lang}"
                    if input_key in demo:
    ...
    # Fallback to any available demonstrations if still no matches
    if not matching_demos:
        print(f"Warning: No cross-lingual demonstrations found. Using any available demonstrations.")
        matching_demos = demonstrations[:min(3, len(demonstrations))]
    
    # Limit to 3 demonstrations
...
def create_demonstration_prompts(demonstrations, template):
    ...
    # Use prompt_input as the base template
...
def task1_metrics(generated_data):
    """
    Compute Task 1 metrics with both global and language-specific standard deviations 
    for binary idiom detection.
    """
    ...
    for key, samples in generated_data.items():
        ...
        try:
            ...
            # Store metrics for calculating global standard deviation
        ...
        # Calculate language-specific standard deviation
...
def task2_metrics(generated_data):
    """
    Compute and save Task 2 metrics with both global and language-specific standard deviations.
    ...
    for key, samples in generated_data.items():
        ...
        try:
            ...
            # Store metrics for global standard deviation
            all_metrics["precision"].append(precision)
            all_metrics["recall"].append(recall)
            all_metrics["f1"].append(f1)
        ...
        # Calculate language-specific standard deviations
